	...
	it('should successfully decode a structured view', (next) => {
		...
		client.readPropertyMultiple(
			...
			(err, response) => {
				...
				expect(response.values[0].objectId).toEqual({
					type: 29,
					instance: 31,
				})
				expect(object[28]).toEqual([
					{
				...
				expect(object[75]).toEqual([
					{ value: { type: 29, instance: 31 }, type: 12 },
				])
				expect(object[77]).toEqual([
					{
				...
				expect(object[168]).toEqual([
					{ value: '7-BA-RDS1-041-SBCv13.20', type: 7, encoding: 0 },
				])
				expect(object[207]).toEqual([
				...
				expect(object[211]).toEqual([
					{
						value: { value: { type: 5, instance: 0 }, type: 12 },
						type: 118,
					},
					{
						value: { value: { type: 3, instance: 0 }, type: 12 },
						type: 118,
					},
				])
	...
	it('should successfully decode a value object', (next) => {
		...
		client.readPropertyMultiple(
			...
			(err, response) => {
				...
				expect(object[111]).toEqual([
					{ value: { value: [0], bitsUsed: 4 }, type: 8 },
				])
				expect(object[168]).toEqual([
					{ value: '7-BA-RDS1-024-SBCv13.20', type: 7, encoding: 0 },
				])
				expect(object[4930]).toEqual([{ value: 0, type: 9 }])
